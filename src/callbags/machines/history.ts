...
import { type AnyMachine, type Machine } from './core'
...
export function productMachine<Context extends AnyTagged = BottomTag>() {
	return function <
		Param,
		Product extends Record<
			PropertyKey,
			Machine<any, any, any, Context, any, any>
		>,
		Select = InferResult<Product>,
		Passthrough extends boolean = false,
		Exit = InferExit<Product> & Tagged<'just', unknown>,
	>(
		product: Product,
		...
		options?: Partial<{
			exit: Init<Exit, [InferExit<Product> & Tagged<'just', unknown>]>
		...
		return {
			...
			send: (
				...
				if (options?.passthrough) {
					...
					let next = s
					for (const [k, m] of Object.entries(product)) {
						const res = m.send(e, next[k], context)
						if (res !== next[k]) {
							dirty = true
							next = { ...next, [k]: res }
						}
...
type T = Tagged<'just', never> extends Tagged<'just', unknown> ? true : false
